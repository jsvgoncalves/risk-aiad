...
public class BoardLabel extends JLabel{
	...
	private BufferedImage label_img;
	BoardLabel(String s, int x, int y, int width, int height, Color back, Color border){
		...
		this.bc = border;
		try {
			label_img = ImageIO.read(BoardGUI.class.getResource("res/label.png"));
		} catch (IOException e) {
	...
	protected void paintComponent(Graphics g) {
		...
		g.drawImage(label_img, 0, 0, this.getWidth(), this.getHeight(), this);
